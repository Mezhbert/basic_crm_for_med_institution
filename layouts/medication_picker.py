...
# ------ Medication_picker Layout ------
class MedicationPickerLayout:
    ...
    def window(self) -> sg.Window:
        ...
        self.__data = db.medication.select_picker()

        # описание окна с информацией о пациенте
        medication_picker_layout = [
            [
                [
                    sg.Text(
                        text="Title:",
                    ...
                    sg.Input(
                        key="-INPUT_MEDICATION_PICKER_FIND_BY_NAME-",
                        enable_events=True,
                        expand_x=True
                    )
                ],
            ...
            [
                sg.Table(
                    values=self.__data,
                    headings=["Title", "Desc", "Contraindication"],
                    max_col_width=25,
            ...
            [
                sg.Button(
                    button_text="OK",
                    key="-BUTTON_MEDICATION_PICKER_OK-",
                    enable_events=True,
                    expand_x=True
                )
            ]
        ...
        # создание window
        self.m_window = sg.Window("Medication picker", medication_picker_layout, resizable=True, finalize=True)
    ...
    def events_handler(self, event, values):
        ...
        # ввод имени в поисковик
        elif event == "-INPUT_MEDICATION_PICKER_FIND_BY_NAME-":
            name = self.m_window["-INPUT_MEDICATION_PICKER_FIND_BY_NAME-"].get()
            ...
            if length == 1:
                # взятие данных из бд
                self.__data = db.medication.find_by_name_picker("{}%".format(name))
            ...
            self.m_window["-TABLE_MEDICATION_PICKER-"].update(self.__data)

        # если клик по таблице
        elif event[0] == "-TABLE_MEDICATION_PICKER-":
            key = event[0]
            action = event[1]
            row, col = event[2]
            ...
            # если клик не по ячейке или заголовку таблицы
                ...
                # снятие выделения
                self.m_window["-TABLE_MEDICATION_PICKER-"].update(select_rows=[])
        ...
        elif event == "-BUTTON_MEDICATION_PICKER_OK-":
            self.m_window.close()
            args = (self.m_name, self.m_parent_layout)
            return ("picked_medication", args)
