...
# ------ Doctor_picker Layout ------
class DoctorPickerLayout:


    def __init__(self, parent_layout : str, arg : str):
    ...
    def window(self) -> sg.Window:
        ...
        self.__data = db.doctor.select_picker()

        # описание окна с информацией о докторе
        doctor_picker_layout = [
            [
                [
                    sg.Text(
                        text="Doctor:",
                    ...
                    sg.Input(
                        key="-INPUT_DOCTOR_PICKER_FIND_BY_NAME-",
                        enable_events=True,
                        expand_x=True
                    )
                ],
        ...
        # создание window
        self.m_window = sg.Window("Doctor picker", doctor_picker_layout, resizable=True, finalize=True)
    ...
    def events_handler(self, event, values):
        ...
        # ввод имени в поисковик
        elif event == "-INPUT_DOCTOR_PICKER_FIND_BY_NAME-":
            name = self.m_window["-INPUT_DOCTOR_PICKER_FIND_BY_NAME-"].get()
            ...
            if length == 1:
                # взятие данных из бд
                self.__data = db.doctor.find_by_name_picker("{}%".format(name))
            ...
            self.m_window["-TABLE_DOCTOR_PICKER-"].update(self.__data)

        # если клик по таблице
        elif event[0] == "-TABLE_DOCTOR_PICKER-":
            key = event[0]
            action = event[1]
            row, col = event[2]
            ...
            # если клик не по ячейке или заголовку таблицы
                ...
                # снятие выделения
                self.m_window["-TABLE_DOCTOR_PICKER-"].update(select_rows=[])
        ...
        elif event == "-BUTTON_DOCTOR_PICKER_OK-":
            self.m_window.close()
            args = (self.m_name, self.m_parent_layout, self.m_arg)
            return ("picked_doctor", args)
