...
# класс для работы с mysql с таблицей докторов
class Doctors(metaclass=Singleton):
    ...
    def insert(self, name : str, birthdate : datetime, job : str, wage : int, phone : str):
    ...
    def update(self, id : int, name : str, birthdate : datetime, job: str, wage : int, phone : str):
        self.m_cursor.execute(
            self.m_sql.get_sql("update"),
            (name, birthdate, job, wage, phone, id)
            )
