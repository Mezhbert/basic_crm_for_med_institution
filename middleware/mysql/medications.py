...
# класс для работы с mysql с таблицей препаратов
class Medications(metaclass=Singleton):
    def __init__(self, db, cursor):
        ...
        self.m_sql = Sql(os.path.join(parent_path, "sql/medications.sql"))
    ...
    def insert(self, title : str, desc : str, contraindication : str):
    ...
    def update(self, id : int, title : str, desc : str, contraindication : str):
        self.m_cursor.execute(
            self.m_sql.get_sql("update"),
            (title, desc, contraindication, id)
            )
    ...
    def find_by_name_picker(self, name : str) -> tuple:
        self.m_cursor.execute(
            self.m_sql.get_sql("find_by_name_without_id"),
            (name,)
            )
    ...
    def select_picker(self):
        self.m_cursor.execute(
            self.m_sql.get_sql("select_without_id")
            )
